...
Two gaps are real and not worth papering over: `1.52.0`, `1.53.0` and `1.58.1`
passed through `package.json` on `dev` without ever being tagged, so their
commits fold into the next tagged section, and `1.9.0` has a section but no tag.
PREAMBLE
    printf '\n## [Unreleased]\n\n'
...
check() {
  ...
  if [[ -n "$extra" ]]; then
    warn "sections with no tag (expected: 1.9.0 was released but never tagged):"
    printf '    %s\n' $extra >&2
  fi
  if [[ -n "$missing" ]]; then
