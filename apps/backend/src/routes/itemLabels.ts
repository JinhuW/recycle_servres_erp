...
// boot cache); this route only writes. Anyone may add a label inline from the
...
const itemLabels = new Hono<{ Bindings: Env; Variables: { user: User } }>();

export const ITEM_LABEL_MAX = 40;

/** Trim and collapse internal runs of whitespace; '' when there's nothing left. */
export function normalizeItemLabel(v: unknown): string {
  return typeof v === 'string' ? v.trim().replace(/\s+/g, ' ') : '';
}

// The management list: retired labels included, plus how many lines carry each
...
itemLabels.post('/', async (c) => {
  const body = (await c.req.json().catch(() => null)) as { name?: unknown } | null;
  const name = normalizeItemLabel(body?.name);
  if (!name) return c.json({ error: 'name required' }, 400);
  if (name.length > ITEM_LABEL_MAX) {
    return c.json({ error: `name must be ${ITEM_LABEL_MAX} characters or fewer` }, 400);
  }

  const sql = getDb(c.env);
  // Idempotent on lower(name): typing "cpu" when "CPU" exists selects the
  // existing label instead of minting a twin. A retired label is revived
  // rather than duplicated.
  const [row] = await sql<{ id: string; name: string }[]>`
    INSERT INTO item_labels (name, created_by)
...
itemLabels.patch('/:id', async (c) => {
  ...
  if (body.name !== undefined) {
    name = normalizeItemLabel(body.name);
    if (!name) return c.json({ error: 'name required' }, 400);
    if (name.length > ITEM_LABEL_MAX) {
      return c.json({ error: `name must be ${ITEM_LABEL_MAX} characters or fewer` }, 400);
    }
  ...
  try {
    ...
    const updated: Updated = await sql.begin(async (tx): Promise<Updated> => {
      const [existing] = await tx<{ id: string; name: string }[]>`
        SELECT id, name FROM item_labels WHERE id = ${id}
      `;
      ...
      // order_lines stores the label name, so a rename has to sweep the lines
      // carrying the old one or history silently splits in two.
      if (name && name !== existing.name) {
        await tx`
          UPDATE order_lines SET item_label = ${name} WHERE item_label = ${existing.name}
        `;
      }
      return row;
    });
