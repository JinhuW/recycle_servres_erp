...
lookups.get('/', async (c) => {
  ...
  const [catalogRows, sourceRows, statusRows, categoryRows, itemTypeRows] = await Promise.all([
    sql`
      SELECT "group", value
  ...
  return c.json({
    ...
    itemTypes: itemTypeRows.map(r => ({ id: r.id as string, name: r.name as string })),
  });
});
