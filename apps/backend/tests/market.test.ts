...
describe('GET /api/market — target margin', () => {
  ...
  it('uses the workspace-configured target_margin for maxBuy', async () => {
    ...
    const def = await api<{ targetMargin: number; items: { avgSell: number; maxBuy: number; lastPrice: number | null }[] }>(
    ...
    const r = await api<{ targetMargin: number; items: { avgSell: number; maxBuy: number; lastPrice: number | null }[] }>(
      'GET', '/api/market', { token });
    expect(r.body.targetMargin).toBe(0.5);
    const row = r.body.items.find(i => i.avgSell > 0)!;
    const basis = row.lastPrice ?? row.avgSell;
    expect(row.maxBuy).toBeCloseTo(+(basis * 0.5).toFixed(2), 2);
